...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if database_seeding:
            ...
            staff_curry = Staff.objects.create(name="Guy Fieri", role="Manager")
            staff_thompson = Staff.objects.create(name="Rachael Ray", role="Chef")
            staff_green = Staff.objects.create(name="Michael Caines", role="Waiter")
            staff_durant = Staff.objects.create(name="Jamie Oliver", role="Waiter")
            staff_ramsay = Staff.objects.create(name="Gordon Ramsay", role="Chef")
            
            # Example seeding for Diner
            diner_giap = Diner.objects.create(name="Nguyen Nhu Giap", email="giap.nn225441@sis.hust.edu.vn", phone_num="1234567890")
            diner_binh = Diner.objects.create(name="Nguyen Duc Binh", email="binh.nd225475@sis.hust.edu.vn", phone_num="0987654321")
            diner_khanh = Diner.objects.create(name="Tran An Khanh", email="khanh.ta225447@sis.hust.edu.vn")
            diner_bao = Diner.objects.create(name="Mai Viet Bao", email="bao.mv225474@sis.hust.edu.vn", phone_num="0123456789")
            diner_tu = Diner.objects.create(name="Pham Anh Tu", email="tu.pa225463@sis.hust.edu.vn", phone_num="0123457789")
            diner_mbappe = Diner.objects.create(name="Kylian Mbappe", phone_num="9876543210")
            diner_messi = Diner.objects.create(name="Lionel Messi")
